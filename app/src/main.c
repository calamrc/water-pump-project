...
#include <string.h> // For memcpy in median filter
...
// Additional constants for fixes
...
static bool pump_on = false; // Moved here for timer handler access
...
static void safety_timer_handler(struct k_timer *timer_id)
{
    ARG_UNUSED(timer_id);
    if (pump_on) {
        int ret = gpio_pin_set(gpio_dev, 22, 1); // OFF
        if (ret < 0) {
...
// Helper function to compute median of an array (needed for median filter)
static int64_t get_median(int64_t *arr, int size) {
    ...
    // Return middle element (size=5, so index 2)
...
int main(void)
{
    ...
    while (1) {
        ...
        if (k_sem_take(&data_sem, timeout) == 0) {
            ...
            if (flow_rate_lpm > 0.0f && period_us > 0) {  // Skip if zero/stale
                if (detect_plateau(flow_rate_lpm)) {
                    ...
                    if (!pump_on) {
                        ...
                        ret = gpio_pin_set(gpio_dev, 22, 0); // ON (low for active low)
                        if (ret < 0) {
            ...
            LOG_INF("Timeout occurred, resetting flow state");
            period_us = 0;
            valid_periods = 0;
            buffer_index = 0;
            memset((void *)period_buffer, 0, sizeof(period_buffer));
