...
int sensor_manager_init(void)
{
    ...
    int ret = gpio_pin_configure(gpio_dev, FLOW_SENSOR_PIN, GPIO_INPUT | GPIO_PULL_UP);
    if (ret < 0) {
    ...
    ret = gpio_pin_interrupt_configure(gpio_dev, FLOW_SENSOR_PIN, GPIO_INT_EDGE_FALLING);
    if (ret < 0) {
    ...
    // Add callback
    gpio_init_callback(&flow_callback, sensor_isr, BIT(FLOW_SENSOR_PIN));
...
{
    // Disable interrupt
    int ret = gpio_pin_interrupt_configure(gpio_dev, FLOW_SENSOR_PIN, GPIO_INT_DISABLE);
    if (ret < 0) {
